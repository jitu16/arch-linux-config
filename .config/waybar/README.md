...
> [!IMPORTANT]
> If you're not using Alacritty, replace all its invocations with your
...
- The leftmost module has no default function and is reserved for custom use.
  You can configure it to run any command. For example:
   ...
   "custom/trigger": {
   	...
   	"on-click": "alacritty --hold sh -c fastfetch",
   	// Run btop
...
- You can define keybinds to interact with modules using their respective
  [scripts](./scripts/). For example:
   ...
   # Interactive
   bind = SUPER, B, exec, alacritty -e ${scr}/bluetooth.sh
   bind = SUPER, N, exec, alacritty -e ${scr}/network.sh
   bind = SUPER, O, exec, alacritty -e ${scr}/power-menu.sh
   bind = SUPER, U, exec, alacritty -e ${scr}/system-update.sh
